...
io.on("connection", (socket) => {
  ...
  socket.on("create_room", async (data) => {
    ...
    const room = {
      ...
      chatLog:[], createdAt:Date.now()/1000, votes:{},
    };
    rooms.set(rid, room);
...
app.get("/mapi/domestic-board", (req, res) => {
  ...
  const S = Math.max(6, Math.min(parseInt(req.query.S || "9"), 12));
  const board = E.generateDomesticBoard(preset, S);
...
app.get("/mapi/random-board", (req, res) => {
  ...
  const board = E.generateRandomBoard(seed, mode, S);
...
httpServer.listen(PORT, "0.0.0.0", () => {
  console.log(`🎲  Monopoly Online v3.1  →  http://0.0.0.0:${PORT}`);
});
